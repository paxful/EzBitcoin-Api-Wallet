	...
	public function address_balance() {
		...
		try {
			if ( $address ) {
				...
				if ( $user_address ) {
					$response = json_encode( array( 'balance'        => $user_address->balance,
					                                'address'        => $address,
					                                'total_received' => $user_address->total_received,
				...
				$response     = json_encode( array( 'balance'        => $user_balance->balance,
				                                    'address'        => $address,
				                                    'total_received' => $user_balance->total_received,
				                                    'crypto_type'    => $this->crypto_type
					) );
				$this->output
					->set_content_type( 'application/json' )
