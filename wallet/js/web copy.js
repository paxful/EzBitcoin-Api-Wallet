...
function jsfunct_GiveThanks(strURL){
	//this function submits thanks+ request and gives feedback
	//$.ajax({ type: "POST", async: false, url: strURL, data: dataString,	success: function(result) {
	$.post(strURL , function(data){
		
			var arr = data.split(',');
	        for(var i = 0; i<arr.length; i++){
				var strDataCode = arr[0];
				var intBalance = arr[1];
	        }
			//jsfunct_Alert('strDataCode=' + strDataCode + ' intBalance=' + intBalance);
			if (strDataCode == 'itemnotfound') { jsfunct_Alert('This file no longer exists'); }
			if (strDataCode == 'hack') { jsfunct_Alert('Administrator Alerted... .... .....'); }
			if (strDataCode == 'givernotfound') { jsfunct_Alert('You must be a member to give thanks with credit',7000); }
			if (strDataCode == 'needcredit') { jsfunct_Alert('You need to earn or buy credit to give thanks +',7000); }
			if (strDataCode == 'alreadythanked') { jsfunct_Alert('You already gave thanks.',7000); }
			if (strDataCode == 'done') { //success
				
				if(intBalance>0){ 
					soundManager.play('thanks','/sounds/gilget.mp3'); //play sound
					document.getElementById('hud_over_exp_points').innerHTML=intBalance ;//update balance ( hud_over_exp_points )
					jsfunct_Alert('Thanks :) credit=' + intBalance );
				}else{
					jsfunct_Alert('Thanks Given');
				}
			}
	});
}
